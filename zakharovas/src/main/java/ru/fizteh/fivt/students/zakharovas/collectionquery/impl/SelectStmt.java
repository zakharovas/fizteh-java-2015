...
import ru.fizteh.fivt.students.zakharovas.collectionquery.impl.aggregators.AggregationFunction;
...
public class SelectStmt<T, R> implements Query<R> {
    ...
    public class WhereStmt<T, R> implements Query<R> {
        ...
        public WhereStmt<T, R> limit(int amount) {
            if (amount < 0) {
                throw new IllegalArgumentException("limit should be not negative number");
            }
            limit = amount;
            return this;
        }
        ...
        public Iterable<R> execute() throws ReflectiveOperationException {
            ...
            if (havingPredicate != null) {
                ...
                for (List<R> element : result) {
                    newResult.add(element.stream().filter(havingPredicate::test).collect(Collectors.toList()));
                }
                result = newResult;
            }
            ...
            if (hasConvertingFunction) {
                ...
                for (Function<T, ?> function : functions) {
                    if (function instanceof AggregationFunction) {
                        Object resultOfFunction = ((AggregationFunction) function).applyOnList(currentSource);
